...
void sendFileList(int client_socket, char *directory) {
    ...
    if (dir == NULL) {
        char *errorMsg = "Erreur ouverture repertoire.";
...
void sendFileContent(int client_socket, char *filename) {
    ...
    if (file == NULL) {
        char *errorMsg = "Erreur ouverture fichier.";
        write(client_socket, errorMsg, strlen(errorMsg));
        return;
    }
...
void sendClientDuration(int client_socket, time_t startTime) {
    ...
    char durationStr[50];
    snprintf(durationStr, sizeof(durationStr), "Durée: %d secondes\n", duration);
...
int main() {
    printf("le serveur est en cours d'exécution");
    int server_fd, client_socket, opt = 1;
    struct sockaddr_in address;
    int addrlen = sizeof(address);

    if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
        perror("Socket echouée");
        exit(EXIT_FAILURE);
    }
